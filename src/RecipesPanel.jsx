...
import {
  ...
  Sparkles, Loader2, Eraser, BookOpen, ChevronLeft
} from 'lucide-react';
...
export default function RecipesPanel({ recipeType }) {
  ...
  const mobileShowingDetail = showingForm || selected;
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-4 lg:grid lg:grid-cols-[280px_1fr] lg:gap-4">
      ...
      <main className={`min-w-0 ${!mobileShowingDetail ? 'hidden lg:block' : 'block'}`}>
        ...
        {mobileShowingDetail && (
          <button
            onClick={() => { setSelectedId(null); setIsAdding(false); setIsEditing(false); }}
...
function SidebarItem({ recipe, isSelected, onSelect, onRename }) {
      ...
      <li>
        <div className={`flex items-center gap-1 px-2 py-1.5 rounded-md ${isSelected ? 'bg-red-50' : 'bg-stone-50'}`}>
          <input
            ...
            onBlur={commit}
            className="flex-1 min-w-0 px-2 py-1.5 text-sm bg-white border border-stone-300 rounded focus:outline-none focus:ring-1 focus:ring-red-700"
          />
          <button
            ...
            className="p-2 text-green-700 hover:bg-green-50 active:bg-green-100 rounded"
          >
          ...
          <button
            ...
            className="p-2 text-stone-500 hover:bg-stone-100 active:bg-stone-200 rounded"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
  ...
  return (
    <li className="group relative">
      ...
      {/* Rename pencil: always visible on mobile/touch, hover-only on desktop */}
      <button
        onClick={startRename}
        title="Rename"
        aria-label="Rename recipe"
        className="absolute right-1 top-1/2 -translate-y-1/2 p-2 text-stone-400 hover:text-stone-900 active:text-stone-950 hover:bg-stone-100 active:bg-stone-200 rounded transition-opacity lg:opacity-0 lg:group-hover:opacity-100 lg:focus-within:opacity-100"
      >
  ...
  return (
    <div className="bg-white rounded-lg border border-stone-200 p-4 sm:p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base sm:text-lg font-bold truncate pr-2">{isAdding ? `New ${typeLabel}` : `Edit: ${selected?.name}`}</h2>
        <button onClick={onCancel} aria-label="Close" className="text-stone-400 hover:text-stone-700 p-1 flex-shrink-0">
          <X className="w-5 h-5" />
        </button>
      </div>
      <div className="space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="flex items-center gap-2 bg-green-800 hover:bg-green-900 active:bg-green-950 disabled:bg-stone-300 disabled:cursor-not-allowed text-white text-sm font-medium px-4 py-2.5 rounded-md transition-colors"
          >
            <Save className="w-4 h-4" />
            Save
          </button>
          <button onClick={onCancel} className="text-sm text-stone-600 hover:text-stone-900 active:text-stone-950 px-4 py-2.5">
            Cancel
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <div className="bg-white rounded-lg border border-stone-200 p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            ...
            <h2 className="text-xl sm:text-2xl font-bold break-words">{selected.name}</h2>
          </div>
          <div className="flex gap-1 flex-shrink-0">
            <button onClick={onEdit} title="Edit" aria-label="Edit recipe" className="p-2.5 text-stone-500 hover:text-stone-900 hover:bg-stone-100 active:bg-stone-200 rounded-md transition-colors">
              <Edit2 className="w-4 h-4" />
            </button>
            <button onClick={onDelete} title="Delete" aria-label="Delete recipe" className="p-2.5 text-stone-500 hover:text-red-700 hover:bg-red-50 active:bg-red-100 rounded-md transition-colors">
      ...
      <div className="bg-white rounded-lg border border-stone-200 overflow-hidden">
        <div className="bg-gradient-to-r from-stone-900 to-stone-800 text-white px-4 sm:px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 rounded-full bg-red-700 flex items-center justify-center flex-shrink-0">
              <ChefHat className="w-4 h-4" />
            </div>
            <div className="min-w-0">
              <div className="text-sm font-semibold truncate">Chef Matteo</div>
              <div className="text-xs text-stone-300 truncate">Bologna · 25 years of panini</div>
            </div>
          </div>
          {history.length > 0 && (
            <button onClick={onClearHistory} title="Clear conversation" aria-label="Clear conversation" className="text-stone-300 hover:text-white text-xs flex items-center gap-1 p-2 flex-shrink-0">
          ...
          {history.length === 0 ? (
            <div className="text-center py-6">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 bg-red-800 hover:bg-red-900 active:bg-red-950 disabled:bg-stone-300 text-white text-sm font-medium px-4 py-2.5 rounded-md transition-colors"
              >
            ...
            <div className="space-y-4 max-h-[50vh] sm:max-h-[500px] overflow-y-auto pr-1 sm:pr-2">
              {history.map((ex, i) => (
          ...
          <div className="mt-4 pt-4 border-t border-stone-100">
            <div className="flex gap-2">
              <textarea
                ...
                placeholder="Tell Matteo what you're thinking..."
                rows={2}
              ...
              <button
                ...
                className="bg-red-800 hover:bg-red-900 active:bg-red-950 disabled:bg-stone-300 disabled:cursor-not-allowed text-white px-4 sm:px-4 rounded-md transition-colors flex items-center justify-center min-w-[48px]"
